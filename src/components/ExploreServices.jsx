  ...
  return (
    <section className="w-full bg-[#f4f6f8] py-12 px-4 md:px-8 lg:px-[70px]">
      <div className="max-w-[1280px] mx-auto">
        ...
        {/* Tabs */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 bg-white border border-black/10 rounded-[4px] overflow-hidden mb-5">
          {services.map((tab) => (
            <button
              key={tab.id}
              type="button"
              onClick={() => setActiveTab(tab.id)}
              className={`min-h-[54px] px-3 flex items-center justify-center text-[14px] md:text-[17px] font-semibold text-center transition ${
                tab.id === activeTab
        ...
        {/* Active Content */}
        <div className="bg-white rounded-[4px] px-4 sm:px-6 md:px-10 py-8 md:py-12 border border-black/10 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div>
            <div className="w-[56px] h-[56px] md:w-[62px] md:h-[62px] rounded-full bg-[#2637e8] flex items-center justify-center text-white text-[24px] md:text-[28px] mb-6 md:mb-10">
              {activeService.icon}
            </div>

            <h3 className="text-[24px] sm:text-[30px] md:text-[38px] font-semibold leading-tight text-[#151515] mb-5 md:mb-7 max-w-[480px]">
              {activeService.title}
            </h3>

            <p className="text-[15px] md:text-[20px] leading-[1.55] md:leading-[1.45] font-medium text-black max-w-[560px] mb-7 md:mb-9">
              {activeService.desc}
            </p>

            <Link
              to={activeService.link}
              className="inline-flex items-center gap-3 text-[15px] md:text-[17px] font-semibold text-black hover:text-[#2637e8] transition"
            >
              Learn More
          ...
          <div className="flex justify-center lg:justify-end">
            <img
              ...
              className="w-full max-w-[420px] h-[220px] sm:h-[280px] md:h-[350px] object-cover rounded-[4px]"
            />
          </div>
        </div>
      </div>
